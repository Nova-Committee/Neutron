...
object InventoryHome {
  def getHomeStack(home: IHome): ItemStack = {
    ...
    stack.setTagDisplayName(home.getName).appendExtraTooltip(TextFormatting.YELLOW + Utilities.L10n.getFromCurrentLang("phr.neutron.position",
      s"DIM${home.getDim} >> ${Utilities.Location.getLiteralFromVec3(home.getPos)}"))
  }
}

class InventoryHome(player: EntityPlayerMP, page: Int) extends InventoryInteraction("ui.neutron.home", 36) with IPageable {
