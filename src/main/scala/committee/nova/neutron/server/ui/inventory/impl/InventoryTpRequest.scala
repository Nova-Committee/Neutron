...
class InventoryTpRequest(server: MinecraftServer, player: EntityPlayerMP, tpid: UUID) extends InventoryInteraction(Utilities.L10n.getFromCurrentLang("ui.neutron.tprequest"), 9) {
  override def init(): Unit = {
    ...
    for (request <- ServerStorage.teleportRequestSet if tpid == request.getId && request.isRelevantTo(player)) {
      ...
      setInventorySlotContents(if (to) 1 else 7, Utilities.Player.getPlayerSkull(Utilities.L10n.getFromCurrentLang("phr.neutron.sender", Utilities.Player.getPlayerNameByUUID(server, request.getSender))))
      setInventorySlotContents(if (to) 7 else 1, Utilities.Player.getPlayerSkull(Utilities.L10n.getFromCurrentLang("phr.neutron.receiver", Utilities.Player.getPlayerNameByUUID(server, request.getReceiver))))
