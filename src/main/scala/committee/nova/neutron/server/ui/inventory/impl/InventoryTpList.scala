...
object InventoryTpList {
  ...
  def getRequestStack(request: ITeleportRequest, player: EntityPlayerMP): ItemStack = {
    ...
    stack.setTagDisplayName(request.getInfo)
      .appendExtraTooltip(if (wasIgnored(request, player)) TextFormatting.GRAY + Utilities.L10n.getFromCurrentLang("phr.neutron.ignored")
      else TextFormatting.YELLOW + Utilities.L10n.getFromCurrentLang("phr.neutron.pending"))
  }
  ...
  def getSwitchStack(current: Int): ItemStack = {
    ...
    stack.setTagDisplayName(Utilities.L10n.getFromCurrentLang("button.neutron.switch",
      ...
      .appendExtraTooltip(TextFormatting.YELLOW + Utilities.L10n.getFromCurrentLang("button.neutron.switchTo",
...
class InventoryTpList(player: EntityPlayerMP, page: Int, filter: ITpListFilter) extends InventoryInteraction(Utilities.L10n.getFromCurrentLang(s"ui.neutron.tplist.${filter.getId}"), 36)
